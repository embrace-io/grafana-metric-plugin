 ...
 * Page Properties
 */
export interface AppConfigProps extends PluginConfigPageProps<AppPluginMeta<GlobalSettings>> {}

/**
 * Config component
 */
const AppConfig: FC<AppConfigProps> = (props) => {
  ...
  const onEnable = async () => {
    ...
    if (dataSourceInfo === undefined && !showErrorMessage) {
      const results = await getBackendSrv().post('/api/datasources', {
    ...
    await getBackendSrv().put(`api/datasources/uid/${dataSourceInfo.uid}`, {
      ...dataSourceInfo,
    ...
    await getBackendSrv()
      .get(`api/datasources/proxy/${dataSourceInfo.id}/api/v1/status/buildinfo`)
      .catch((error) => {
        if (error.status === 405) {
          setValidToken(true);
        } else {
  ...
  return (
    <React.Fragment>
      ...
      <p>
        The Embrace App is a plugin for Grafana that gives teams an easy way to visualize Embrace dashboards using
        Embrace Metrics API data powered by Prometheus.
      </p>
      {!isEnabled && (
        <React.Fragment>
          <p>
            Get out-of-the-box Embrace dashboards in minutes with our simple configuration. Enter your Embrace API Token
            below and click to Enable the Application.
          </p>
          <InlineField
      ...
      <div className="gf-form gf-form-button-row">
        {isEnabled ? (
          ...
          <Button onClick={onEnable} disabled={apiToken === ''}>
            Enable
          </Button>
        )}
      </div>
